...
def item_quantity_price(typeID,quantity, regionID=10000002, systemID=0):
    """
    Pulls the price for a specific quantity of items using order depth
 
    Args:
        typeID: TypeID of the item to pull the price for
        quantity: the amount of items to be bought.
        regionID: The region from which the orders should be pulled.
        systemID: (Optional) Limits the orders to the system the order was made in.
    ...
    Raises:
        ValueError: Not enough orders to fill the specified quantity
    """
    price = 0
    order = 0
    df = order_depth(typeID, regionID, systemID)
    while quantity > 0:
        if quantity > df['Volume'].sum():
            raise ValueError("Your order cannot be filled. Choose a lower quantity and/or a wider order range.")
            break
        if quantity < df['Volume'].iloc[order]:
            price += (quantity * df['Price'].iloc[order])
            quantity = 0
            return price
        if quantity > df['Volume'].iloc[order]:
            price += (df['Price'].iloc[order] * df['Volume'].iloc[order])
            quantity -= df['Volume'].iloc[order]    
            order += 1  
            continue
    return price
...
def market_history(typeID,column='all', regionID=10000002, date = False,days=400):
    """
    Pulls the market history from the ESI for a specific typeID in a region for a maximum of 400 days
 
    Args:
        typeID: TypeID of the item to pull the market history for
        regionID: The region from which the orders should be pulled.
        column: The data to output
        date: Specifies whether to output a the date as well
        days: Timeframe for which to pull the market history
 
    Returns:
        Returns a dataframe with the market history for a typeID in a specified timeframe
 
    Raises:
        TypeError: Entered a negative timeframe or specified an invalid column type
    """
    if column not in ['highest','lowest','average','order_count','volume','all']:
        raise TypeError("That's not a valid column type. Supported names are: 'highest','lowest','average','order_count','volume','all'")
    if days <= 0:
    ...
    if date == False:
        ...
        if column in ['highest','lowest','average','order_count','volume']:
            return df[column]
    if date == True:
        if column in ['highest','lowest','average','order_count','volume']:    
            return df[[column,'date']]
    if column == 'all':
      return df
